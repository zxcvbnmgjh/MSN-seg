...
def standardize(img): # 对输入图像进行标准化
    mean = th.mean(img)
    std = th.std(img)
    img = (img - mean) / std
    return img
...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    ...
    if schedule_name == "linear":
        ...
        # diffusion steps.
        # 线性调度，beta 从 scale * 0.0001 线性增加到 scale * 0.02
...
def betas_for_alpha_bar(num_diffusion_timesteps, alpha_bar, max_beta=0.999):
    # 根据给定的 alpha_bar(t) 函数反向推导出 betas 序列
    """
...
class ModelMeanType(enum.Enum):
    ...
    PREVIOUS_X = enum.auto()  # the model predicts x_{t-1}，模型预测 x_{t-1}
    START_X = enum.auto()  # the model predicts x_0，模型预测 x_0
    EPSILON = enum.auto()  # the model predicts epsilon，模型预测 噪声 ε
...
class ModelVarType(enum.Enum):
    ...
    LEARNED = enum.auto()  # 模型直接学习方差
    FIXED_SMALL = enum.auto() # 使用固定的、较小的方差
    FIXED_LARGE = enum.auto() # 使用固定的、较大的方差
    LEARNED_RANGE = enum.auto() # 模型学习一个介于两者之间的值
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        alphas = 1.0 - betas
        self.alphas_cumprod = np.cumprod(alphas, axis=0) # alphas_cumprod：α 的累积乘积，即 ᾱ_t
    ...
    def q_sample(self, x_start, t, noise=None):
        # 从 q(x_t | x_0) 中采样 x_t ，实现前向扩散，给 x_0 添加噪声得到 x_t
        """
