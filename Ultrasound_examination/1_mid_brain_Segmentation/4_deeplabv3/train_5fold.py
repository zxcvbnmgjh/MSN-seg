...
def get_transform_train(crop_size=512, mean=(0.16882014, 0.16886712, 0.16884266), std=(0.2274119, 0.22743388, 0.22739229)):
    return SegmentationPresetTrain(crop_size=crop_size, mean=mean, std=std)
def get_transform_val(crop_size=512, mean=(0.16882014, 0.16886712, 0.16884266), std=(0.2274119, 0.22743388, 0.22739229)):
...
def main(args, fold_idx, all_indices, full_dataset):
    ...
    # 保存文件名到文件
    save_idx_dir = "./5fold_save_result/fold_indices"
    ...
    results_dir = f"./5fold_save_result/results_fold{fold_idx}"
    os.makedirs(results_dir, exist_ok=True)
    results_dir_path = os.path.join(results_dir, "train_metrics.txt")

    save_dir = f"./save_weights/5fold/fold_{fold_idx}"
    os.makedirs(save_dir, exist_ok=True)
...
def parse_args():
    ...
    parser.add_argument("--epochs", default=300, type=int, metavar="N")  # 建议从 600 降至 300
    ...
    parser.add_argument("--fold", default=None, type=int, choices=[0,1,2,3,4],
                        help="Specify which fold to train (0-4). If not set, train all folds.") #指定训练哪一轮
...
if __name__ == '__main__':
    ...
    if args.fold is not None:
        # 只训练指定的 fold
        print(f"\n{'=' * 20} Training ONLY Fold {args.fold} {'=' * 20}")
        fold_info = main(args, args.fold, indices, full_dataset)
