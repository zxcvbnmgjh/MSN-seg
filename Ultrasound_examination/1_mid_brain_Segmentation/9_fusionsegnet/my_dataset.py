...
class Midbrain_Full(Dataset):
    
    def __init__(self, root: str, transforms=None):
        super(Midbrain_Full, self).__init__()
        self.flag = "full"  # 载入所有数据集
        data_root = os.path.join(root, "0_midbrain_data", self.flag)
...
class Midbrain_Test(Dataset):
    
    def __init__(self, root: str, transforms=None):
        super(Midbrain_Test, self).__init__()
        self.flag = "full"  # 载入所有数据集
        data_root = os.path.join(root, "0_midbrain_data", self.flag)
        assert os.path.exists(data_root), f"path '{data_root}' does not exist."
