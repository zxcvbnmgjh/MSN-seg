...
def criterion(inputs, target, loss_weight=None, num_classes: int = 2, dice: bool = True):
    # inputs为字典dict[str, torch.Tensor]；target为标签图torch.Tensor
    losses = {}
    for name, x in inputs.items(): # 对字典inputs进行遍历
        # 一个字典（dict），通常包含模型的多个输出头（head）；字典方法，返回(key, value)对列表

        unique_values = torch.unique(target)
        # print(f"Unique values in target: {unique_values}")

        target = torch.where(target == 255, torch.tensor(1, device=target.device), target)
...
def evaluate(model, data_loader, device, num_classes):
    ...
    with torch.no_grad():
        for image, target in metric_logger.log_every(data_loader, 100, header):
            ...
            output = model(image) # 返回的是字典形式的output
            output = output['out']
            print("Target unique:", torch.unique(target))  # 应该包含 0 和 1（或更多类别）         
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, num_classes,
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header): # 遍历数据加载器 data_loader，每次处理一个批次的数据
        ...
        with torch.amp.autocast(device_type='cuda', enabled=scaler is not None):
            ...
            """
            output = output['out']
            print("Output shape:", output.shape)  # 应该是 [B, C, H, W]
            print("Output sample:", output[0, :, 0, 0])  # 查看第一个样本的第一个像素的类别概率
            print("Target shape:", target.shape)  # 应该是 [B, H, W]
            print("Target dtype:", target.dtype)  # 应该是 torch.int64
            print("Target unique:", torch.unique(target))  # 应该包含 0 和 1（或更多类别）
            pred = output.argmax(dim=1)
            print("Pred shape:", pred.shape)  # [B, H, W]
            print("Pred unique:", torch.unique(pred))  # 应该包含前景类（如 1）
            print("Pred sum:", pred.sum())  # 如果一直是 0，说明预测全是背景类
            """
...
def evaluate_loss(model, data_loader, device, num_classes):
    ...
    model.eval()
    confmat = utils.ConfusionMatrix(num_classes) #  混淆矩阵对象，用于记录预测结果与真实标签之间的匹配情况
    dice = utils.DiceCoefficient(num_classes=num_classes) # Dice 系数对象，用于计算平均 Dice 系数(此处计算的是soft dice)
    metric_logger = utils.MetricLogger(delimiter="  ")  #日志工具，用于记录和显示评估过程中的信息
    header = 'val:'
    if num_classes == 2: # 如果是二分类任务（num_classes == 2），定义一个权重张量 [1.0, 2.0]，其中背景类权重为 1.0，前景类权重为 2.0。
        # 设置cross_entropy中背景和前景的loss权重(根据自己的数据集进行设置)
...
def evaluate_loss(model, data_loader, device, num_classes):
    # 验证集损失：dice_loss + CE_loss（criterion里定义）
    # 验证集dice：hard dice（pred mask vs gt mask）
    model.eval()
    ...
    eps = 1e-6
    # 累加每类：intersection（TP）以及 pred_sum + target_sum（分母的一部分）
    inter = torch.zeros(num_classes, device=device, dtype=torch.float64)
    denom = torch.zeros(num_classes, device=device, dtype=torch.float64)
    ...
    with torch.no_grad():
        for image, target in metric_logger.log_every(data_loader, 100, header):
            ...
            loss = criterion(output_dict, target, loss_weight, num_classes=num_classes)
            metric_logger.update(loss=loss.item())

            logits = output_dict["out"]          # [B, C, H, W]
            pred = logits.argmax(dim=1)          # [B, H, W]  hard mask
            ...
            for c in range(num_classes):
                pred_c = (pred == c)
                targ_c = (target == c)
                inter[c] += (pred_c & targ_c).sum().to(torch.float64)
                denom[c] += (pred_c.sum() + targ_c.sum()).to(torch.float64)

    dice_per_class = (2.0 * inter + eps) / (denom + eps)

    # 常见做法：不把背景(0)算进平均dice；二分类就取前景(1)
    if num_classes > 1:
        hard_dice = dice_per_class[1:].mean().item()
    else:
        hard_dice = dice_per_class.mean().item()

    confmat.reduce_from_all_processes()

    return metric_logger.meters["loss"].global_avg, confmat, hard_dice
