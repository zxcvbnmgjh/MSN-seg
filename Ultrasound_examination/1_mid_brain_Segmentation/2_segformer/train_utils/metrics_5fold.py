...
import os
from sklearn.metrics import precision_score, recall_score # 移除了 f1_score
from scipy.spatial.distance import cdist


def calculate_assd(pred_mask, true_mask):
    """
    计算 Average Symmetric Surface Distance (ASSD)。
    """
    # 获取前景像素的坐标
    coords_pred = np.argwhere(pred_mask == 1)
    coords_true = np.argwhere(true_mask == 1)

    # 如果任一掩码为空，ASSD 无法计算，返回 inf
    ...
    # 找到每个预测边界点到最近真实边界点的距离
    dist_pred_to_true = np.min(dist_matrix, axis=1)
    # 找到每个真实边界点到最近预测边界点的距离
    dist_true_to_pred = np.min(dist_matrix, axis=0)

    # 计算对称的平均表面距离
    assd = (np.mean(dist_pred_to_true) + np.mean(dist_true_to_pred)) / 2.0
...
def calculate_metrics(pred_mask, true_mask):
    ...
    # ========== 计算 ASSD (Average Symmetric Surface Distance) ==========
...
def calculate_test_metrics(model, data_loader, device, save_dir=None):
    ...
    # ========== 计算平均指标（关键：ASSD 和 HD95 跳过 inf）==========
    avg_metrics = {}
    for key in metrics_list[0].keys():
        values = [m[key] for m in metrics_list]

        if key in ["Hausdorff_Distance 95%", "ASSD"]:
            # 过滤掉 inf（空 mask 导致）
