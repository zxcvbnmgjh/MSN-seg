...
def criterion(inputs, target, loss_weight=None, num_classes: int = 2, dice: bool = True):
    ...
    # 如果 inputs 是张量，则包装成字典
    if isinstance(inputs, torch.Tensor):
        inputs = {"out": inputs}

    for name, x in inputs.items():
        unique_values = torch.unique(target)
        # print(f"Unique values in label map: {unique_values}")

        # 断言确保没有非法值，但允许 ignore_index=255 存在
        assert all((unique_values >= 0) & (unique_values < num_classes) | (unique_values == 255)), \
            f"Label contains invalid value(s): {unique_values}"

        # 使用 ignore_index=255 来忽略这些区域
        ce = F.cross_entropy(x, target, weight=loss_weight, ignore_index=255)

        dsc = 0.
        if dice:
            # 构建 one-hot 编码并跳过 255
            dice_target = build_target(target, num_classes=num_classes, ignore_index=255)
            dsc = dice_loss(x, dice_target, multiclass=True)
    ...
    if len(losses) == 1:
        ...
        return losses['out'] + 0.5 * losses.get('aux', 0)
...
def evaluate(model, data_loader, device, num_classes):
    ...
    with torch.no_grad():
        for image, target in metric_logger.log_every(data_loader, 100, header):
            ...
            # print(output).print("val Target shape:", target.shape)  # 应该是 [B, H, W]
            # print("val Target dtype:", target.dtype)  # 应该是 torch.int64
            # print("val Target unique:", torch.unique(target))  # 应该包含 0 和 1（或更多类别）
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, num_classes,
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header): # 遍历数据加载器 data_loader，每次处理一个批次的数据
        ...
        with torch.amp.autocast('cuda', enabled=(scaler is not None)):
            ...
            out = output['out']
            print(out)
            print("Output shape:", out.shape)  # 应该是 [B, C, H, W]
            print("Output sample:", out[0, :, 0, 0])  # 查看第一个样本的第一个像素的类别概率
            print("train Target shape:", target.shape)  # 应该是 [B, H, W]
            print("train Target dtype:", target.dtype)  # 应该是 torch.int64
            print("train Target unique:", torch.unique(target))  # 应该包含 0 和 1（或更多类别）
