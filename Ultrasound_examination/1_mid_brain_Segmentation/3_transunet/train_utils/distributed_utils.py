...
class DiceCoefficient(object):
    ...
    def update(self, pred, target):
        ...
        # pred = F.softmax(pred, dim=1)  # 添加这一行！否则 pred 不是概率分布
        pred = F.one_hot(pred.argmax(dim=1), self.num_classes).permute(0, 3, 1, 2).float()
        # 对于一张预测结果图，针对每一个像素找到所属概率最大的类别，作为其预测数值，然后转为one hot编码形式（对于每个类别的groundtruth）
        dice_target = build_target(target, self.num_classes)
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        i = 0 # 初始化批次计数器，表示当前处理的是第几个批次
        if not header:
            header = ''
        start_time = time.time() 
        end = time.time()
        iter_time = SmoothedValue(fmt='{avg:.4f}') # 记录每个批次的迭代时间（包括前向传播、反向传播等）
        data_time = SmoothedValue(fmt='{avg:.4f}') # 记录数据加载时间
