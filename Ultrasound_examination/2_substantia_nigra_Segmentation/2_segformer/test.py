...
def main(args):
    ...
    # 将结果保存到文件
    result_file = os.path.join(os.path.dirname(args.resume), "B3_test_result.txt")
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description="pytorch unet training")
    parser.add_argument("--data-path", default="/data2/gaojiahao/Ultrasound_examination/Segmentation/segformer-plpa", help="DRIVE root")
    # 修改最优权重路径
    parser.add_argument('--resume', default='/data2/gaojiahao/Ultrasound_examination/Segmentation/segformer-plpa/save_weights/best_model.pth', help='resume from checkpoint')
    parser.add_argument("--num-classes", default=1, type=int)
