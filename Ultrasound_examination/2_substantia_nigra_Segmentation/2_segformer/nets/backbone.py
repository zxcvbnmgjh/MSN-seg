...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768): # 实际embed_dim不是768，每层的embed_dim即Ci由参数设置
        super().__init__()
...
#   当输入图片为512, 512时，Block1的特征图为128, 128，此时就先将特征层压缩为16, 16。(在Block1的Attention模块中，相当于将8x8个特征点进行特征浓缩，浓缩为一个特征点。)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.kv         = nn.Linear(dim, dim * 2, bias=qkv_bias) # 线性层的作用是将输入张量从维度 dim 映射到 2 * dim，其中前一半表示 Key (K)，后一半表示 Value (V)
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape # 输入的形状为 bs, 16384, 32
        # bs, 16384, 32 => bs, 16384, 32 => bs, 16384, 8, 4 => bs, 8, 16384, 4 （bs, 8, 16384, 4：输入经过query线性变换之后得到的query向量形状）
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        if self.sr_ratio > 1:
            # bs, 16384, 32 => bs, 32, 128, 128
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
            # bs, 32, 128, 128 => bs, 32, 16, 16 => bs, 256, 32 (将128*128的特征图特征浓缩为16*16)
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
            x_ = self.norm(x_)
            # bs, 256, 32 => bs, 256, 64 => bs, 256, 2, 8, 4 => 2, bs, 8, 256, 4（bs, 8, 256, 4：输入经过key、value线性变换之后得到的key和value向量形状）
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        else:
            kv = self.kv(x).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # 使用 self.kv 得到 Key 和 Value 的拼接结果,将其 reshape 成 (2, B, num_heads, N_kv, head_dim) 形状,然后通过索引 kv[0] 和 kv[1] 分别取出 Key 和 Value
        k, v = kv[0], kv[1]
        ...
        # bs, 16384, 32 => bs, 16384, 32 输入图像经过attention操作之后形状不变
        x = self.proj(x)
        x = self.proj_drop(x)
...
def drop_path(x, drop_prob: float = 0., training: bool = False, scale_by_keep: bool = True):
    ...
    """
    if drop_prob == 0. or not training: #如果 drop_prob 是 0 或者当前不是训练阶段
        return x
    keep_prob       = 1 - drop_prob # 存活概率（保留路径的概率）
    shape           = (x.shape[0],) + (1,) * (x.ndim - 1)  # 对于每个样本，都会生成一个相同大小的掩码，但只在第一个维度（batch 维度）上有不同的值，其余维度都是 1
    random_tensor   = x.new_empty(shape).bernoulli_(keep_prob) # random_tensor 能够自动扩展到与 x 相同的形状，对于第一个维度（batch 维度），每个样本都有一个独立的随机值来决定是否丢弃该样本路径；对于其余维度，每个元素都会使用对应 batch 样本的相同随机值进行缩放或丢弃操作。
    if keep_prob > 0.0 and scale_by_keep:
        random_tensor.div_(keep_prob) # 如果启用 scale_by_keep=True，会对保留的样本乘以 1 / keep_prob，确保输出期望值不变
...
class DWConv(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = self.dwconv(x) # 深度卷积层，每个输入通道都独立地进行卷积
        x = x.flatten(2).transpose(1, 2) # X:(B,C,H,W)->(B,C,N)->(B,N,C)
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=GELU, drop=0.):
        ...
        self.fc1    = nn.Linear(in_features, hidden_features) # 第一个全连接层，将输入映射到更高维空间（或隐藏层）
        self.dwconv = DWConv(hidden_features) # 深度可分离卷积层，增强局部特征提取能力
        self.act    = act_layer() # 激活函数
        
        self.fc2    = nn.Linear(hidden_features, out_features) # 第二个全连接层，将高维表示映射回输出维度
        
        self.drop   = nn.Dropout(drop) # Dropout 层，用于正则化
    ...
    def forward(self, x, H, W):
        ...
        x = self.dwconv(x, H, W) # [B, N, hidden_features]->[B, hidden_features, H, W]->[B, N, hidden_features]
        x = self.act(x)
        x = self.drop(x)
...
class MixVisionTransformer(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[32, 64, 160, 256],
        ...
        #-----------------------------------------------#
        self.patch_embed1 = OverlapPatchEmbed(patch_size=7, stride=4, in_chans=in_chans, embed_dim=embed_dims[0]) # H*W*C->(H/4)*(W/4)*C1
        #-----------------------------------------------#
        ...
        #-----------------------------------------------#
        self.patch_embed2 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[0], embed_dim=embed_dims[1]) # (H/4)*(W/4)*C1->(H/8)*(W/8)*C2
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        #   4096, 64 => 4096, 64
        #-----------------------------------------------#
        ...
        #-----------------------------------------------#
        self.patch_embed3 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[1], embed_dim=embed_dims[2]) # (H/8)*(W/8)*C2->(H/16)*(W/16)*C3
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        #   1024, 160 => 1024, 160
        #-----------------------------------------------#
        ...
        #-----------------------------------------------#
        self.patch_embed4 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[2], embed_dim=embed_dims[3]) # (H/16)*(W/16)*C3->(H/32)*(W/32)*C4
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        #   256, 256 => 256, 256
        #-----------------------------------------------#
