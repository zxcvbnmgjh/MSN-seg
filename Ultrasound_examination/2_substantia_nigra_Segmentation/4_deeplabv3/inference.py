...
def load_model(model_path, num_classes=2, aux=False, device="cuda:0"):
    """加载训练好的 DeepLabV3 模型"""
...
def single_model_predict(model, dataloader, device, save_dir):
    ...
    for images, img_paths in dataloader:
        ...
        # 前向推理 (DeepLabV3 输出为 dict)
        output_dict = model(images)  # {'out': [B, 2, H, W], 'aux': ...}
        ...
        if isinstance(output_dict, dict):
            if "out" in output_dict:
                ...
                for v in output_dict.values():
                    ...
                    raise TypeError(f"Unexpected DeepLabV3 output: {output_dict.keys()}")
        else:
            logits = output_dict
...
def evaluate_png_folders(pred_folder: str, label_folder: str, output_file: str, spacing=None):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("DeepLabV3 Model Segmentation Metrics Report\n")
        f.write("=" * 80 + "\n")
...
# ============================= 主函数 =============================
def main():
    parser = argparse.ArgumentParser(description="DeepLabV3 inference for substantia nigra segmentation")
    ...
    parser.add_argument("--test-names-file", type=str,
                        ...
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/4_deeplabv3/5fold_save_result/fold_indices/fold_4_test.txt")
    parser.add_argument("--weights", type=str,
                        help="Path to SegFormer model checkpoint (.pth)",
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/4_deeplabv3/save_weights/5fold/fold_4/best_model.pth")
    parser.add_argument("--aux", action="store_true",
                        help="Use auxiliary loss during training (default: False)")
    ...
    parser.add_argument("--pred-save-dir", type=str, 
                        ...
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/4_deeplabv3/predict_results/predicts/iteration5")
    parser.add_argument("--metrics-output", type=str,
                        help="Path to save metrics report (e.g., ./metrics.txt)",
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/4_deeplabv3/predict_results/test_metrics/iteration5")
    ...
    # Step 4: Load trained DeepLabV3 model
    model = load_model(
    ...
    print(f"Loaded DeepLabV3 model from {args.weights}")
