...
def draw_contours_on_image(image, contours, color, thickness=10):
    """
    在图像上绘制轮廓。
...
def process_and_overlay_gt_contours(image_folder, mask_folder, output_folder):
    ...
    for prefix in sorted(common_prefixes):
        ...
        try:
            ...
            # 3. 在原始图像上绘制绿色轮廓（真实标签）
            overlay = draw_contours_on_image(image, contours, color=(0, 0, 255), thickness=1)  # 红色
...
if __name__ == "__main__":
    # --- 请修改为你的实际路径 ---
    image_folder = '/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/0_sn_data/full/images_512' # 原图文件夹路径
    mask_folder = '/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/0_sn_data/full/masks_512'  # 掩码文件夹路径
    output_folder = '/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/0_sn_data/full/overlapped_labels_512' # 结果路径
