...
def load_model(model_path, num_classes=2, vit_name="R50-ViT-B_16", 
               img_size=512, n_skip=3, vit_patches_size=16, device="cuda:0"):
    """加载训练好的 TransUNet 模型"""
    # 加载配置
...
def single_model_predict(model, dataloader, device, save_dir):
    ...
    for images, img_paths in dataloader:
        ...
        # 前向推理 (TransUNet 直接输出 [B, C, H, W])
        output_dict = model(images)  # [B, 2, H, W]
...
def evaluate_png_folders(pred_folder: str, label_folder: str, output_file: str, spacing=None):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("TransUNet Model Segmentation Metrics Report\n")
        f.write("=" * 80 + "\n")
...
# ============================= 主函数 =============================
def main():
    parser = argparse.ArgumentParser(description="TransUNet inference for substantia nigra segmentation")
    ...
    parser.add_argument("--vit_name", type=str, default="R50-ViT-B_16",
                        help="TransUNet variant (e.g., R50-ViT-B_16, ViT-B_16)")
    parser.add_argument("--img_size", type=int, default=512,
                        help="Input image size (must match training)")
    parser.add_argument("--n_skip", type=int, default=3,
                        help="Number of skip connections (default: 3)")
    ...
    parser.add_argument("--test-names-file", type=str,
                        ...
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/3_transunet/5fold_save_result/fold_indices/fold_4_test.txt")
    parser.add_argument("--weights", type=str,
                        help="Path to SegFormer model checkpoint (.pth)",
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/3_transunet/save_weights/5fold/fold_4/best_model.pth")
    parser.add_argument("--gt-mask-dir", type=str, 
                        help="Directory of ground truth masks (for evaluation)",
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/0_sn_data/full/splits_1/fold4/masks")
    parser.add_argument("--pred-save-dir", type=str, 
                        help="Directory to save predicted masks",
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/3_transunet/predict_results/predicts/iteration5")
    parser.add_argument("--metrics-output", type=str,
                        help="Path to save metrics report (e.g., ./metrics.txt)",
                        default="/data2/gaojiahao/Ultrasound_examination/2_substantia_nigra_Segmentation/3_transunet/predict_results/test_metrics/iteration5")
    ...
    # Step 4: Load trained TransUNet model
    model = load_model(
    ...
    print(f"Loaded TransUNet-{args.vit_name} model from {args.weights}")
