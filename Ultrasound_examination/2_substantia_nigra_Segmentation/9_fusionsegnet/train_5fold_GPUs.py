...
def parse_args():
    ...
    parser.add_argument("--gpus", default="1,2", type=str, help="GPU IDs to use, e.g., '0,1'")
    args = parser.parse_args()
    return args


def setup(rank, world_size, port='29501'):
    """初始化分布式环境"""
...
def run_fold_training(rank, world_size, args, fold, indices, full_dataset):
    """在指定 GPU 上运行 fold 训练"""
    port = str(29500 + fold + 1)  # 每个 fold 使用不同端口避免冲突
...
if __name__ == '__main__':
    ...
    for fold in range(total_folds):
        ...
        mp.spawn(
            run_fold_training,
            args=(world_size, args, fold, indices, full_dataset),
            nprocs=world_size,
            join=True
        )
