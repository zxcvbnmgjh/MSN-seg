...
class ECALayer(nn.Module):
    def __init__(self, channels: int, gamma: int = 2, b: int = 1):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1) # 将 H×W 的空间维度压缩为 1×1，保留通道信息
        self.conv1d = nn.Conv1d(1, 1, kernel_size=k_size, padding=k_size // 2, bias=False) # 1D 卷积
        self.sigmoid = nn.Sigmoid()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.avg_pool(x).squeeze(-1).transpose(-1, -2) # (B, C, H, W) → (B, C, 1, 1)→ (B, 1, C)
        y = self.conv1d(y).transpose(-1, -2).unsqueeze(-1) # (B, 1, C) → (B, C, 1, 1)
        scale = self.sigmoid(y)
        return x * scale # 增强重要通道，抑制不重要通道
