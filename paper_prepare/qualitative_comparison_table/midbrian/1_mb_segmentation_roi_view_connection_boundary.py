...
# 参数设置
# ===========================
image_dir = "/data2/gaojiahao/paper_prepare/qualitative_comparison_table/midbrian/data_together_2"
...
# 从 JSON 文件加载 ROI 坐标
# ===========================
roi_file = "/data2/gaojiahao/paper_prepare/qualitative_comparison_table/midbrian/rois.json"  # 确保与脚本同目录或指定路径
...
for i, img_id in enumerate(image_ids):
    ...
    # -------- 第二列：标签放大图（原输入图像 → 换成标签图像）--------
    axes[i, 1].axis('off')
    try:
        # 获取标签文件路径，标签文件名以 _label 结尾
        label_path = os.path.join(image_dir, f"{img_id}_label{suffix}")
        label_img = load_image(label_path, mode="RGB")
        
        if label_img is not None:
            # 根据 ROI 坐标裁剪标签图像
            roi_label = label_img[roi_y:roi_y+roi_h, roi_x:roi_x+roi_w]
            axes[i, 1].imshow(roi_label)
        ...
        print(f"Error cropping label for {img_id}: {e}")
        axes[i, 1].text(0.5, 0.5, "N/A", ha='center', va='center',
                        color='red', fontsize=10)
...
output_path = "/data2/gaojiahao/paper_prepare/qualitative_comparison_table/midbrian/mb-segmentation_comparison_new_roi_view_connection.png"
